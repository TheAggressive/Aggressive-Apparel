...
ci_php 'composer install --no-interaction --prefer-dist --no-progress'
ci_php 'find includes -name "*.php" -exec php -l {} \; >/dev/null && echo "PHP syntax valid"'
ci_php 'composer lint:php'
ci_php 'XDEBUG_MODE=off ./vendor/bin/phpstan analyse --memory-limit=2G --verbose'
ci_php 'XDEBUG_MODE=coverage ./vendor/bin/phpunit --testsuite=unit --coverage-clover=coverage-unit.xml'
