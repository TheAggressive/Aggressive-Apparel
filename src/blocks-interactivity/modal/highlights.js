...
export const findBlockDomElement = clientId => {
  ...
  const editorIframes = [
    // Site editor canvas
    ...Array.from(document.querySelectorAll('iframe[name="editor-canvas"]')),
    // Template part editor frames and template editor frames
    ...Array.from(document.querySelectorAll('.edit-site-visual-editor iframe')),
...
export const cleanupAllHighlights = (modalId = null) => {
  ...
  const findAndCleanElements = (rootElement = document) => {
    ...
    rootElement.querySelectorAll(allHighlightSelector).forEach(element => {
  ...
  try {
    ...
    Debug.add(`Error cleaning up highlights in iframe: ${error.message}`, true);
  }
