...
$icon_style = isset( $attributes['iconStyle'] ) ? sanitize_key( (string) $attributes['iconStyle'] ) : 'solid';
$stroke     = isset( $attributes['iconStrokeWidth'] ) ? (float) $attributes['iconStrokeWidth'] : 1.75;
...
if ( ! in_array( $icon_style, array( 'solid', 'outline' ), true ) ) {
	$icon_style = 'solid';
}

$stroke = max( 1, min( 3, $stroke ) );
...
// %.2F is locale-independent (always "." decimal separator).
$color_style = sprintf( '--aa-dark-mode-toggle-icon-stroke-width:%.2F;', $stroke );
...
$wrapper_attributes = array(
	'class' => sprintf(
		'is-size-%s has-text-align-%s is-icon-style-%s',
		$size,
		$alignment,
		$icon_style
	),
...
<div <?php echo wp_kses_post( get_block_wrapper_attributes( $wrapper_attributes ) ); ?>>
	...
	>
		<span class="dark-mode-toggle__icons" aria-hidden="true">
			<?php if ( 'outline' === $icon_style ) : ?>
				<svg class="dark-mode-toggle__icon dark-mode-toggle__icon--sun" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" focusable="false">
					<circle cx="12" cy="12" r="4" />
					<path d="M12 2v2M12 20v2M4.93 4.93l1.41 1.41M17.66 17.66l1.41 1.41M2 12h2M20 12h2M4.93 19.07l1.41-1.41M17.66 6.34l1.41-1.41" />
				</svg>
				<svg class="dark-mode-toggle__icon dark-mode-toggle__icon--moon" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-linecap="round" stroke-linejoin="round" focusable="false">
