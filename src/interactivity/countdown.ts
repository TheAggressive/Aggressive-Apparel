...
/* ------------------------------------------------------------------ */
/*  Dynamic countdown support                                          */
/* ------------------------------------------------------------------ */

const DYNAMIC_INIT_FLAG = 'aaCountdownInitialised';

/**
 * Track every dynamic countdown's end timestamp and the segment elements
 * that need their textContent updated each tick. WeakRef would be nicer
 * but isn't available in the IA module's lib target — instead we GC by
 * pruning entries whose root has been removed from the document.
 */
...
function tickDynamicCountdowns(): void {
  ...
  for (let i = dynamicCountdowns.length - 1; i >= 0; i--) {
    ...
    // Drop entries whose card has been removed from the DOM (e.g. by
    // re-render after a filter change) so they stop being ticked.
...
/**
 * Discover any `[data-aa-countdown]` elements within `root` that haven't
 * been wired up yet, register them with the shared ticker, and run the
 * first tick immediately so values reflect the live time on first paint.
 */
